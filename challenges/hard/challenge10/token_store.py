...
The challenge server (and the boot scripts) materialize a ``tokens.json`` file
for the demo. Historically this file — and the ``authenticate`` tool's return
value — were seeded with hardcoded JWTs checked into source control
(CWE-798 / CWE-312), which leaked real-looking admin/service/user tokens into
the repository and into every container image built from it.
