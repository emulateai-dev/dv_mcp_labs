"""Centralised credential access for Challenge 6.

This module is the ONLY place Challenge 6 obtains the API keys shown in the
internal memo and the ``internal://credentials`` resource. The live production
keys previously embedded directly in ``server.py`` / ``server_sse.py`` (and thus
committed to source control, CWE-798 / CWE-312) have been removed.

Defaults returned here are deliberately non-functional placeholder values so
the deliberately-vulnerable challenge still behaves the same way during
play/exercises. A real deployment supplies the actual values through the
``DVMCP_CHALLENGE6_*`` environment variables below; secrets must never live in
source control.
"""
...
# Clearly non-functional placeholder defaults. These keep the challenge
# behaviour intact without exposing any real credential in source control.
...
# Map logical credential names to the env var a deployment uses to supply them.
ENV_VAR_MAP = {
...
def get_internal_memo() -> str:
    """Return the internal security-audit memo.

    The memo is templated with runtime credentials so no live key is hardcoded
    in source control.
    """
...
def get_api_keys() -> str:
    """Return the system API keys resource body.

    The keys are templated with runtime credentials so no live key is
