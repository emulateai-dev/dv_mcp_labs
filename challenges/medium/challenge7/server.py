...
try:
    from .token_store import write_tokens as _write_tokens, DEFAULT_TOKENS_PATH
...
# Create a directory for token storage and materialize tokens.json from the
# runtime token store. Secrets are sourced from DVMCP_CHALLENGE7_* environment
# variables (with non-functional placeholder defaults) so no real credential
# is checked into source control (CWE-798 / CWE-312).
TOKEN_FILE = DEFAULT_TOKENS_PATH
