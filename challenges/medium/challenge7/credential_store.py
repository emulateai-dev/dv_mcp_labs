"""Runtime credential provisioning for Challenge 7 (Token Theft).

The Challenge 7 servers (``server.py`` and ``server_sse.py``) historically
embedded live-looking API keys, JWT access tokens, refresh tokens and an admin
API token directly in source control (CWE-798 / CWE-312). On first boot the
stdio server also wrote those secrets verbatim into
``/tmp/dvmcp_challenge7/tokens.json``, leaking them into every container image
built from the repository.

This module keeps operational secrets out of source control. Each value is
read from a deployment-supplied ``DVMCP_CHALLENGE7_*`` environment variable and
falls back to a clearly-marked, non-functional placeholder value only when no
override is configured — so the challenge still boots in a fresh checkout
...
# ---------------------------------------------------------------------------
# Safe, non-functional demo defaults.
#
# These are intentionally obviously-fake values so the challenge remains
# runnable in a sandbox/lab without shipping real secrets in source control.
# They must never be treated as valid in any production-style check.
# ---------------------------------------------------------------------------
SAFE_DEFAULT_TOKENS = {
    "email_service": {
...
# Per-(service, field) environment variable overrides. Operators deploy real
# secrets through these rather than embedding them in the repository.
ENV_VAR_MAP = {
...
def _resolve(field_name: str, field: str) -> str:
    """Return the runtime value for a (service, field), falling back to the
    safe demo default when no environment variable is set."""
    env_value = os.getenv(ENV_VAR_MAP[(field_name, field)])
    if env_value:
        return env_value
    return SAFE_DEFAULT_TOKENS[field_name][field]
